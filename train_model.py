...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    ...
    if model_type == 'UNet':
        logger.info('Use model: {}'.format(model_type))
        model = UNet(n_channels=config.n_channels, n_classes=config.n_labels)

    elif model_type == 'SPC':
        logger.info('Use model: {}'.format(model_type))
        model = SPC(n_channels=config.n_channels, n_classes=config.n_labels)

    else: raise TypeError('Please enter a valid name for the model type')


    model = model.cuda()
    logger.info(torchsummary.summary(model, (3, 224, 224)))
    # if torch.cuda.device_count() > 1:
    #     print ("Let's use {0} GPUs!".format(torch.cuda.device_count()))
    # model = nn.DataParallel(model, device_ids=[0])
    criterion = WeightedDiceBCE(dice_weight=0.5, BCE_weight=1)
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=lr)
    if config.cosineLR is True:
        lr_scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=20, T_mult=1, eta_min=1e-5)
    else:
        lr_scheduler =  None
    if tensorboard:
